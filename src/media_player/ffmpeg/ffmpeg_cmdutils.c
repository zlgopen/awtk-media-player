...
double parse_number_or_die(const char* context, const char* numstr, int type, double min,
                           double max) {
  char* tail;
  const char* error;
...
static const OptionDef* find_option(const OptionDef* po, const char* name) {
  ...
  while (po->name) {
    if (!strncmp(name, po->name, len) && strlen(po->name) == len) break;
    po++;
  }
  return po;
}
...
static int write_option(void* optctx, const OptionDef* po, const char* opt, const char* arg) {
  ...
  if (po->flags & OPT_EXIT) exit_program(0);
...
int parse_option(void* optctx, const char* opt, const char* arg, const OptionDef* options) {
  ...
  if (!po->name && opt[0] == 'n' && opt[1] == 'o') {
    ...
    if ((po->name && (po->flags & OPT_BOOL))) arg = "0";
  } else if (po->flags & OPT_BOOL)
    arg = "1";

  if (!po->name) po = find_option(options, "default");
  if (!po->name) {
...
void parse_options(void* optctx, int argc, char** argv, const OptionDef* options,
  ...
  while (optindex < argc) {
    ...
    if (handleoptions && opt[0] == '-' && opt[1] != '\0') {
      ...
      if ((ret = parse_option(optctx, opt, argv[optindex], options)) < 0) exit_program(1);
      optindex += ret;
    } else {
      if (parse_arg_function) parse_arg_function(optctx, opt);
    }
  }
}
...
int locate_option(int argc, char** argv, const OptionDef* options, const char* optname) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (!po->name && cur_opt[0] == 'n' && cur_opt[1] == 'o') po = find_option(options, cur_opt + 2);

    if ((!po->name && !strcmp(cur_opt, optname)) || (po->name && !strcmp(optname, po->name)))
      return i;

    if (!po->name || po->flags & HAS_ARG) i++;
  }
  return 0;
}
...
static void check_options(const OptionDef* po) {
  while (po->name) {
    if (po->flags & OPT_PERFILE) av_assert0(po->flags & (OPT_INPUT | OPT_OUTPUT));
    po++;
  }
}
...
void parse_loglevel(int argc, char** argv, const OptionDef* options) {
  ...
  if (!idx) idx = locate_option(argc, argv, options, "v");
  if (idx && argv[idx + 1]) opt_loglevel(NULL, "loglevel", argv[idx + 1]);
...
int opt_default(void* optctx, const char* opt, const char* arg) {
  ...
  if (!strcmp(opt, "debug") || !strcmp(opt, "fdebug")) av_log_set_level(AV_LOG_DEBUG);

  if (!(p = strchr(opt, ':'))) p = opt + strlen(opt);
...
static int match_group_separator(const OptionGroupDef* groups, int nb_groups, const char* opt) {
  ...
  for (i = 0; i < nb_groups; i++) {
    ...
    if (p->sep && !strcmp(p->sep, opt)) return i;
  }
 ...
 */
static void add_opt(OptionParseContext* octx, const OptionDef* opt, const char* key,
                    const char* val) {
...
static void init_parse_context(OptionParseContext* octx, const OptionGroupDef* groups,
                               int nb_groups) {
  ...
  if (!octx->groups) exit_program(1);

  for (i = 0; i < octx->nb_groups; i++) octx->groups[i].group_def = &groups[i];
...
#define GET_ARG(arg)                                                          \
  do {                                                                        \
    ...
    if (opt[0] == 'n' && opt[1] == 'o' && (po = find_option(options, opt + 2)) && po->name &&
        po->flags & OPT_BOOL) {
...
int opt_cpuflags(void* optctx, const char* opt, const char* arg) {
  ...
  if ((ret = av_parse_cpu_caps(&flags, arg)) < 0) return ret;
...
static int init_report(const char* env) {
  ...
  if (!envlevel) report_file_level = FFMAX(report_file_level, prog_loglevel);
  ...
  av_log(NULL, AV_LOG_INFO,
         ...
         "Report written to \"%s\"\n"
         "Log level: %d\n",
         program_name, tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min,
         tm->tm_sec, filename.str, report_file_level);
...
int opt_timelimit(void* optctx, const char* opt, const char* arg) {
  ...
  if (setrlimit(RLIMIT_CPU, &rl)) perror("setrlimit");
#else
...
static void print_program_info(int flags, int level) {
  ...
  if (flags & SHOW_COPYRIGHT)
    av_log(NULL, level, " Copyright (c) %d-%d the FFmpeg developers", program_birth_year,
           CONFIG_THIS_YEAR);
  av_log(NULL, level, "\n");
...
static unsigned get_codecs_sorted(const AVCodecDescriptor*** rcodecs) {
  ...
  while ((desc = avcodec_descriptor_next(desc))) nb_codecs++;
  if (!(codecs = av_calloc(nb_codecs, sizeof(*codecs)))) {
    av_log(NULL, AV_LOG_ERROR, "Out of memory\n");
    exit_program(1);
  }
  desc = NULL;
  while ((desc = avcodec_descriptor_next(desc))) codecs[i++] = desc;
...
static void print_codecs_for_id(enum AVCodecID id, int encoder) {
  ...
  while ((codec = next_codec_for_id(id, codec, encoder))) printf("%s ", codec->name);
...
int show_codecs(void* optctx, const char* opt, const char* arg) {
  ...
  for (i = 0; i < nb_codecs; i++) {
    ...
    if (strstr(desc->name, "_deprecated")) continue;
...
static void print_codecs(int encoder) {
  ...
  for (i = 0; i < nb_codecs; i++) {
    ...
    while ((codec = next_codec_for_id(desc->id, codec, encoder))) {
      ...
      if (strcmp(codec->name, desc->name)) printf(" (codec %s)", desc->name);
  ...
  while ((filter = av_filter_iterate(&opaque))) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      for (j = 0; pad && avfilter_pad_get_name(pad, j); j++) {
        if (descr_cur >= descr + sizeof(descr) - 4) break;
...
int show_layouts(void* optctx, const char* opt, const char* arg) {
  ...
  for (i = 0; !av_get_standard_channel_layout(i, &layout, &name); i++) {
    if (name) {
      printf("%-14s ", name);
      for (j = 1; j; j <<= 1)
        if ((layout & j)) printf("%s%s", (layout & (j - 1)) ? "+" : "", av_get_channel_name(j));
      printf("\n");
    }
  }
  return 0;
...
static void show_help_demuxer(const char* name) {
  ...
  if (fmt->extensions) printf("    Common extensions: %s.\n", fmt->extensions);

  if (fmt->priv_class) show_help_children(fmt->priv_class, AV_OPT_FLAG_DECODING_PARAM);
}
...
static void show_help_muxer(const char* name) {
  ...
  if (fmt->extensions) printf("    Common extensions: %s.\n", fmt->extensions);
  if (fmt->mime_type) printf("    Mime type: %s.\n", fmt->mime_type);
  ...
  if (fmt->priv_class) show_help_children(fmt->priv_class, AV_OPT_FLAG_ENCODING_PARAM);
}
...
static void show_help_bsf(const char* name) {
  ...
  PRINT_CODEC_SUPPORTED(bsf, codec_ids, enum AVCodecID, "codecs", AV_CODEC_ID_NONE, GET_CODEC_NAME);
...
int show_sources(void* optctx, const char* opt, const char* arg) {
  ...
  if ((ret = show_sinks_sources_parse_arg(arg, &dev, &opts)) < 0) goto fail;

  do {
    fmt = av_input_audio_device_next(fmt);
    if (fmt) {
      if (!strcmp(fmt->name, "lavfi")) continue;  // it's pointless to probe lavfi
      if (dev && !av_match_name(dev, fmt->name)) continue;
