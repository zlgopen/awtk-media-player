...
static int32_t audio_encoder_mp3_encode(audio_encoder_t* encoder, const void* buff, uint32_t size) {
  ...
  channels = (aencoder->config.wave.channels == PCM_MONO)  ? 1 : 2;
  samples_per_pass = shine_samples_per_pass(aencoder->mp3);
  bytes_per_pass = samples_per_pass * sizeof(uint16_t) * channels;
  ...
  for(i = 0; i < n; i++) {
    ...
    p += samples_per_pass * channels; 
  }

  return n * bytes_per_pass;
}

static ret_t audio_encoder_mp3_set_prop(audio_encoder_t* encoder, const char* name, const value_t* v) {
  audio_encoder_mp3_t* aencoder = (audio_encoder_mp3_t*)encoder;
...
static ret_t audio_encoder_mp3_get_prop(audio_encoder_t* encoder, const char* name, value_t* v) {
  ...
  if(tk_str_eq(name, AUDIO_ENCODER_PROP_CHANNELS)) {
    uint32_t channels = value_uint32(v); 
    if(aencoder->config.wave.channels == PCM_MONO) {
      value_set_int(v, 1);
    } else {
      value_set_int(v, 2);
    }
    ...
    value_set_int(v, aencoder->config.wave.samplerate);
    return RET_OK;
  }
